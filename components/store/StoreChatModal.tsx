...
export const StoreChatModal: React.FC<StoreChatModalProps> = ({ 
    ...
    // Filter messages: customers only see their own conversation, admins see all
    const displayMessages = useMemo(() => {
        if (!isCustomerView) return messages; // Admin sees all messages
        if (!normalizedUserEmail) return []; // Not logged in, no messages
        // Customer sees only their messages and admin replies to them
        return messages.filter(msg => 
            msg.customerEmail?.toLowerCase() === normalizedUserEmail ||
            (msg.sender === 'customer' && msg.authorEmail?.toLowerCase() === normalizedUserEmail)
        );
    }, [messages, isCustomerView, normalizedUserEmail]);
