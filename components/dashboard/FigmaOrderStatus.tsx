        ...
        {orderItems.map((item, index) => (
          <div key={index} className="min-h-[48px] h-auto sm:h-12 bg-white dark:bg-gray-800 rounded-lg shadow-[0px_2px_9.6px_0px_rgba(0,0,0,0.08)] flex items-center px-2 sm:px-3 py-2 sm:py-0">
            ...
            <div className="flex-1 ml-2 min-w-0">
              <div className="text-black dark:text-gray-300 text-xs sm:text-[13px] font-normal font-['Poppins'] truncate">{item.label}</div>
            </div>
            <div className="text-black dark:text-white text-lg sm:text-xl lg:text-[28px] font-medium font-['Poppins'] flex-shrink-0 ml-1">{item.value}</div>
          </div>
        ))}
      </div>
    </div>
