  ...
  useEffect(() => {
    ...
    const fetchVisitorData = async () => {
      try {
        ...
        // Fetch visitor stats and online count in parallel
        const [statsRes, onlineRes] = await Promise.all([
          fetch(`${apiUrl}/api/visitors/${tenantId}/stats?period=7d`),
          fetch(`${apiUrl}/api/visitors/${tenantId}/online`)
        ]);
        ...
        if (statsRes.ok && onlineRes.ok) {
          ...
          if (statsData.chartData && Array.isArray(statsData.chartData)) {
            const formattedChartData = statsData.chartData.map(item => ({
        ...
        setStats({ onlineNow: 0, todayVisitors: 0, totalVisitors: 0, last7Days: 0, pageViews: 0 });
        const fallbackData = Array.from({ length: 7 }, (_, i) => {
          const d = new Date();
          d.setDate(d.getDate() - (6 - i));
          return {
    ...
    const interval = setInterval(async () => {
      try {
        ...
        const isLocal = hostname.includes('localhost');
        const apiUrl = isLocal ? 'http://localhost:5001' : `${window.location.protocol}//${hostname.split('.').slice(-2).join('.')}`;
        const onlineRes = await fetch(`${apiUrl}/api/visitors/${tenantId}/online`);
        if (onlineRes.ok) {
          const onlineData = await onlineRes.json();
